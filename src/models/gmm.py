...
class GMM:
    ...
    def _multivariate_gaussian(self, X, mu, cov):
        """Pure NumPy implementation of Multivariate Normal PDF"""
        n = X.shape[1]
        diff = X - mu
        # Add small epsilon to diagonal for numerical stability 
        cov_stable = cov + 1e-6 * np.eye(n)
        
        try:
            det = np.linalg.det(cov_stable)
            inv = np.linalg.inv(cov_stable)
        except np.linalg.LinAlgError:
            return np.zeros(X.shape[0])

        exponent = -0.5 * np.sum(diff @ inv * diff, axis=1)
        normalization = 1.0 / np.sqrt(((2 * np.pi) ** n) * det)
    ...
    def _e_step(self, X):
        ...
        for i in range(self.k):
            ...
            likelihood[:, i] = self._multivariate_gaussian(X, self.mu[i], cov)

        # updated weights or P(Xi|Zi=j)
        numerator = likelihood * self.phi
        sum_likelihood = np.sum(numerator, axis=1)
        denominator = sum_likelihood[:, np.newaxis] + 1e-10
        ...
        self.log_likelihood = np.sum(np.log(sum_likelihood + 1e-10))
